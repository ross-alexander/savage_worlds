...
def apply_advancement(current, advancement):
    ...
    if 'attributes' in advancement:
        for attr in advancement['attributes']:
            ...
            print("Attribute: %s (%d)" % (attr, attributes[attr]['die']))
            change_list.append("Attribute: %s (%d)" % (attr, attributes[attr]['die']))

    # Update current before applying hinderances
    ...
    if 'hinderances' in advancement:
        ...
        for h in advancement['hinderances']:
            if 'advancement' in h:
                adv = h['advancement']
                adv['name'] = "Hinderance: %s" % h['name']
                apply_advancement(current, adv)
        ...
        for k,v in advancement['skills'].items():
            ...
            for i in range(0, 6):
                ...
                if d <= stat_die:
                    ...
                    cost += 2
#                print("%s: d: %d i: %d c: %d" % (k, d, initial, cost))

            change_list.append("Skill: %s (%d)" % (k, die))
            print("Skill: %-20s die=%d initial=%d stat=%d cost=%d" % (k, die, initial, stat_die, cost))
            skills[k] = {
                'die': v['die'],
    ...
    if 'edges' in advancement:
        ...
        [ print("Edge: %s" % e) for e in advancement['edges'] ]
...
def format_swade(opts):
    ...
    if 'gear' in swade:
        ...
        for g in swade['gear']:
            ...
            # Shields, Armour & Weapons
            
            if 'cover' in g:
                print("Shield: %s" % (g['name']))
                shield_list.append(g)

            if 'armour' in g:
                print("Armour: %s" % (g['name']))
                armour_list.append(g)

            if 'damage' in g:
                print("Weapon: %s" % (g['name']))
                weapon_list.append(g)
    ...
    if len(shield_list):
        char_table['shields'] = shield_list
    if len(armour_list):
        char_table['armour'] = armour_list
    if len(weapon_list):
        char_table['weapons'] = weapon_list
